...
import com.example.myapp.entity.CK;
import com.example.myapp.entity.CKNumber;
import com.example.myapp.entity.CKReport;
import com.example.myapp.entity.domain.*;
import com.example.myapp.metric.MetricCompute;
import com.example.myapp.utils.LKUtil;
import com.example.myapp.utils.cyComplexity.antlr4.CyclomaticComplexityVisitor;
import com.example.myapp.utils.cyComplexity.grammar.JavaLexer;
import com.example.myapp.utils.cyComplexity.grammar.JavaParser;
import com.example.myapp.utils.cyComplexity.visulization.ChartPaint;
...
import static com.example.myapp.utils.WebUtil.falseReturn;
import static com.example.myapp.utils.WebUtil.trueReturn;
...
//            String path = "./src/main/resources/codeToBeMetric";
            String destPath = new File(upload).getAbsolutePath();
            // 如果目录不存在则创建
    ...
    ResponseEntity<byte[]> getImage(@RequestParam String path) {
        ...
        byte[] imageContent;
        if (file.exists()) {
            imageContent = fileToByte(output + path);
        } else {
